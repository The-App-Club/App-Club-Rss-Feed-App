...
(async () => {
  ...
  const res = await p('https://flutterawesome.com/');
  ...
  const itemInfoList = [
    ...
    .map((dom) => {
      return {
        feedURL: `https://flutterawesome.com${dom.getAttribute('href')}rss`,
        tagName: dom.textContent,
      };
    });

  await writeFileSync(`${dumpDir}/index.json`, JSON.stringify(itemInfoList));
})();
